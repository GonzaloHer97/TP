...
public class MapComponent extends JPanel implements TrafficSimObserver {
	...
	private void drawRoads(Graphics g) {
		for (Road r : _map.getRoads()) {
			...
			int idx = r.getDestJunc().getCurrGreen();
			if (idx != -1 && r.equals(r.getDestJunc().getRoads().get(idx))) {
			...
			int roadColorValue = 200 - (int) (200.0 * Math.min(1.0, (double) r.getTotalCont() / (1.0 + (double) r.getContLimit())));
	...
	private void drawVehicles(Graphics g) {
		for (Vehicle v : _map.getVehicles()) {
			if (v.getState() != VehicleStatus.ARRIVED) {
				...
				double relLoc = roadLength * ((double) v.getLocate()) / ((double) r.getLength());
